...
- Суффикс `?` на теге документа: `!emit?` = обязательный `$when` + `$yield?:` без `$else-yield`; `!emit-foreach?` / `!emit-range?` = пара `$yield?:`, `$when` опционален. Один закон `?` или два? Тегов `!foreach?` / `!range?` / `!match?` нет (`?:` на ключе поля / `$yield?:` на `!match`).
